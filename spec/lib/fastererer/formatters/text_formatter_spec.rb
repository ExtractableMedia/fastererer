...
describe Fastererer::Formatters::TextFormatter do
  ...
  describe '#render' do
    ...
    context 'with unparsable files' do
      let(:unparsable) { Fastererer::ErrorData.new('bad.rb', 'Err', 'boom') }

      before { formatter.render(report(inspected: 1, unparsable: [unparsable])) }

      it 'routes the parse-error block to stderr', :aggregate_failures do
        expect(err.string).to include('Unprocessable files were:')
