...
describe Fastererer::Offense do
  describe '#initialize' do
    it 'raises UnknownRuleError when offense_name is unknown' do
      expect { described_class.new(:no_such_rule, 1) }
        .to raise_error(Fastererer::UnknownRuleError)
    end
  ...
  describe '#explanation' do
    ...
    it 'shares one Explanation across offenses with the same name' do
      first = described_class.new(:for_loop_vs_each, 1)
      second = described_class.new(:for_loop_vs_each, 2)

      expect(first.explanation).to be(second.explanation)
    end
  end
end
